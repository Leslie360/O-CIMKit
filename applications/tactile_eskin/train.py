...
from applications.tactile_eskin.dataset import get_tactile_dataloader
from applications.tactile_eskin.model import TactileEskinCIMClassifier
...
def main():
    parser = argparse.ArgumentParser(description="Hardware-Aware Tactile E-Skin Braille Recognition")
    parser.add_argument("--epochs", type=int, default=15, help="Number of training epochs")
    parser.add_argument("--batch-size", type=int, default=32, help="Batch size")
    parser.add_argument("--lr", type=float, default=0.01, help="Learning rate")
    parser.add_argument("--noise-level", type=float, default=0.15, help="Friction noise level for tactile inputs")
    ...
    log_path = os.path.join(project_root, "applications", "tactile_eskin", "train.log")
    ...
    print("=" * 60)
    print("CIM Platform - Multimodal Tactile E-Skin Braille Classifier")
    print("=" * 60)
    if memristor_profile:
        print(f"  Loaded Conv Device: {memristor_profile.device_name} ({memristor_profile.discrete_states_count} states)")
    if alox_profile:
        print(f"  Loaded FC Readout Device: {alox_profile.device_name} ({alox_profile.discrete_states_count} states)")
    print(f"  Training for {args.epochs} epochs (Batch Size: {args.batch_size}, LR: {args.lr})")
    print(f"  Input Noise Level: {args.noise_level}")
    print("-" * 60)
    sys.stdout.flush()
    
    # Get Data Loaders
    train_loader, test_loader = get_tactile_dataloader(batch_size=args.batch_size, noise_level=args.noise_level)
    
    t0 = time.time()
    
    # --- PHASE 1: IDEAL SOFTWARE FLOAT BASELINE ---
    print("📢 Phase 1: Training Pure Software Float Baseline (Ideal Float)...")
    model_float = TactileEskinCIMClassifier(
    ...
    model_hw = TactileEskinCIMClassifier(
        memristor_profile=memristor_profile,
        alox_profile=alox_profile,
        n_channels=16,
        out_channels=32,
    ...
    print(f"🏆 Final Tactile E-Skin Accuracy: {best_acc_hw:.2f}%")
